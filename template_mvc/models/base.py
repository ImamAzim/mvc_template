...
class ZemaxApp(object):
    ...
    def __init__(self, path=None):
        ...
        if isInitialized:
            ...
            raise PythonStandaloneApplication2.InitializationException("Unable to locate Zemax OpticStudio.  Try using a hard-coded path.")
        ...
        if self.TheConnection is None:
            raise PythonStandaloneApplication2.ConnectionException("Unable to initialize .NET connection to ZOSAPI")

        self.TheApplication = self.TheConnection.CreateNewApplication()
        if self.TheApplication is None:
            raise PythonStandaloneApplication2.InitializationException("Unable to acquire ZOSAPI application")

        if self.TheApplication.IsValidLicenseForAPI == False:
            raise PythonStandaloneApplication2.LicenseException("License is not valid for ZOSAPI use")

        self.TheSystem = self.TheApplication.PrimarySystem
        if self.TheSystem is None:
            raise PythonStandaloneApplication2.SystemNotPresentException("Unable to acquire Primary system")
...
if __name__ == '__main__':
    ...
    # Insert Code Here
    path = r'C:\Users\usmanii\zemax_projects\neo900p\camera_adapter_4k\camera_adapter_4k.zmx'
    TheSystem.LoadFile(path, False)
    TheLDE = TheSystem.LDE
    Surface_1 = TheLDE.GetSurfaceAt(1)
    print(Surface_1.Thickness)
